...
def send_sms(phone_number, send_count):
    """
    向指定手机号发送短信,循环使用不同设备 User-Agent。
    :param phone_number: 目标手机号
    :param send_count: 用户输入的发送次数
    """
    print(f"\n开始向 {phone_number} 发送短信... 📩")
    print(f"设定发送次数: {send_count} 🔢")
...
if __name__ == "__main__":
    try:
        ...
        if phone_number and phone_number.isdigit() and len(phone_number) == 11:
            send_count = input("请输入发送次数: ").strip()
            try:
                send_count = int(send_count)
                if send_count <= 0:
                    raise ValueError("发送次数必须为正数")
                send_sms(phone_number, send_count=send_count)
            except ValueError:
                print("❌ 请输入有效的发送次数!")
        else:
            print("❌ 请输入有效的11位手机号!")
